...
// Imprint is 320 distinct values across 692 books, so nearly every shelf it built
// would hold one book. It describes the publisher, which is not a taste.
...
export function rankedKinds(profile) {
...
// Scored books first, and unscored ones only to pad a thin shelf. This is the
// listed-versus-scored problem the feed has, answered for this screen: a reader
// landing on a shelf should see books the profile has an opinion about, and the
// 418 catalogue entries with no review behind them should not push them off it.
function pick(entries) {
  const scored = entries.filter((e) => !e.listedOnly);
  const rest = entries.filter((e) => e.listedOnly);
  return [...scored, ...rest].slice(0, MAX_PER_SHELF);
}
...
function tagShelves(books, kinds) {
  ...
  for (const kind of kinds) {
    ...
    for (const [label, entries] of groups) {
      const scored = entries.filter((e) => !e.listedOnly);
      if (scored.length < MIN_BOOKS) continue;
      out.push({
        id: `${kind}:${label}`,
        kind,
...
function standingShelves(books) {
  const out = [];
  ...
  const dated = books
    .filter((e) => !e.listedOnly && e.lastReviewed)
    .sort((a, b) => Date.parse(b.lastReviewed) - Date.parse(a.lastReviewed));
  if (dated.length >= MIN_BOOKS) {
    out.push({
      id: 'latest', kind: 'standing', title: 'Reviewed this week',
      note: 'The most recent reviews the crawl found.',
...
export function buildShelves(books, profile, { limit = 7 } = {}) {
  const all = books || [];
  if (!all.length) return [];

  const standing = standingShelves(all);
  const tagged = tagShelves(all, rankedKinds(profile));
  ...
  const ranked = tagged.sort((a, b) => {
    const sa = a.books.filter((e) => !e.listedOnly).length;
    const sb = b.books.filter((e) => !e.listedOnly).length;
    return sb - sa;
  });

  // One shelf per dimension before a second from any of them, so a reader with a
  // strong subject weight does not get five subject shelves and nothing else.
  const seen = new Set();
  const spread = [];
  ...
  for (const shelf of ranked) {
    if (!spread.includes(shelf)) spread.push(shelf);
  }
