...
import { firebaseConfig } from './firebase-config.mjs?v=47f9c4b88f';
import { mergeProfile, stamp } from './merge-profile.mjs?v=47f9c4b88f';
...
export async function saveDevice(uid, id, subscription, frequency = 'weekly') {
...
// Changing how often a reader wants to hear from a device it has already
// subscribed. A merge rather than a re-subscribe: the push endpoint and keys
// are the browser's, not something this page should be re-sending just to
// flip a preference next to them.
export async function setDeviceFrequency(uid, id, frequency) {
  const { store } = await load();
  await store.updateDoc(deviceRef(store, uid, id), { frequency });
}
