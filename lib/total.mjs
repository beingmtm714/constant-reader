...
export function weightedSum(dims, profile) {
  ...
  // How much of what fired is actually about the book.
  //
  // Form, Scale and Provenance are read off the catalogue record - a page count,
  // a publisher, a shelving category - and they fire on very nearly everything.
  // Counting them as evidence would mean a book looked well-understood because
  // we knew who published it, which is the trap the old 6.9 cap existed to catch
  // and the one a calibration fixture caught again the day these dimensions
  // started firing on every row. So evidence is only the dimensions that read
  // what somebody wrote: Structure, Subject, Register and Tone.
  const evidenceWeight = fired.filter((d) => !d.fromRecord).reduce((sum, d) => sum + d.weight, 0);

  const evidence = profile.evidenceRule || { minDimensions: 5, minWeight: 55 };
  // Renormalising is the licence to let a few dimensions speak for the whole
  // book, and only what somebody wrote can grant it. A page count and an imprint
  // are not a reading however many points they carry.
  const renormalised = evidenceWeight >= evidence.minWeight;
  ...
  return { raw, fired, firedWeight, evidenceWeight, renormalised };
}
...
export function totalFrom({ dims, profile, adjustmentPoints = 0, excluded = false }) {
  const { raw: base, fired, firedWeight, evidenceWeight, renormalised } = weightedSum(dims, profile);
  ...
  // Shrunk against what was actually read, not against what was looked up.
  const pulled = shrink(base, evidenceWeight, profile.prior);
  ...
  return { total, raw, fired, firedWeight, evidenceWeight, renormalised, partialEvidence, proseFloorApplied,
